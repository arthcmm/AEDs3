...
- **Operação Mais Difícil**
  - A integração da tabela hash extensível foi a operação mais desafiadora. Compreender o funcionamento interno da classe `HashExtensivel` e garantir que ela interagisse corretamente com a classe `Arquivo` exigiu um estudo cuidadoso.
...
Apesar dos desafios encontrados, especialmente na compreensão e adaptação da classe `HashExtensivel`, conseguimos alcançar todos os objetivos propostos. Este projeto nos preparou para futuros desenvolvimentos que envolvem sistemas de armazenamento e recuperação eficientes de dados.

**Possíveis Melhorias**

- Implementar tratamento de exceções mais robusto, oferecendo feedback mais detalhado em caso de erros.
- Expandir o sistema para gerenciar outras entidades além de Tarefa, aproveitando a natureza genérica da classe `Arquivo`.
- Implementar uma interface gráfica para facilitar a interação do usuário com o sistema.

**Agradecimentos**

Gostaríamos de agradecer ao professor pela disponibilização da classe `HashExtensivel`, que foi fundamental para a conclusão deste trabalho.
